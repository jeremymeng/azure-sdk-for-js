 ...
 * prep-samples.js
 *
 * Prepares sample files for execution in CI by replacing abosolute package imports/requires with relative
 * imports/requires. This is useful because it allows us to check in "camera-ready" copies of our samples
 ...
 *
 * The command expects to find a directory tree `samples/javascript` under PACKAGE PATH.
 *
...
/**
 * Breadth-first search for files ending in .js, starting from `jsDir`
 *
 * @param {string} jsDir The root of the sample tree to search
 * @returns
 */
async function* findAllTsFiles(jsDir) {
  const initialFiles = await fs.readdir(jsDir, { withFileTypes: true });

  // BFS Queue and queue index
  const q = initialFiles.map(f => [f, jsDir]);
...
/**
 * Replaces package imports/requires with relative imports/requires for CI
 *
...
async function enableLocalRun(fileName, baseDir, pkgName) {
  ...
  const stringToReplace = `require("${pkgName}");`

  const index = fileContents.indexOf(stringToReplace);

  if (index === -1) {
    throw new Error(`Sample ${fileName} did not contain an require statement!`);
  }
  ...
  const relativeImportPath = new Array(depth).fill("..").join("/");
  const updatedContents = fileContents.replace(
    stringToReplace,
    `require("${relativeImportPath}");`
  );
...
async function main() {
  ...
  for await (const fileName of findAllTsFiles(jsDir)) {
    console.log("[prep-samples] Updating requires in", fileName);
