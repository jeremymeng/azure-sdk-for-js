...
- Install the repository dependencies with `pnpm install`
...
2. **Generate code**
    ...
    **Re-generate an existing TypeScript RLC library**
    ...
    ---  
    **NOTE**
    The version of `@azure-tools/typespec-ts` is configured in [eng/emitter-package.json](../eng/emitter-package.json) and the related lock file [eng/emitter-package-lock.json](../eng/emitter-package-lock.json). Change them locally if you need to validate against a different emitter version.
