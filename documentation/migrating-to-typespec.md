...
Install `tsp-client` by running the following from the repo root:
...
3. **Build and test:** `pnpm turbo build --filter=<package-name>... --token 1` from the repo root, then run `pnpm test` from the package directory
