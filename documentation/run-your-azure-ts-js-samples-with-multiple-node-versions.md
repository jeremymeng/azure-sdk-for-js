...
5. run `npx dev-tool samples check-node-versions --artifact-path <artifact path> --node-versions "22"` or you can use any other combination of supported node versions (the repository requires Node.js >=22) including specifying the full semver for it.
