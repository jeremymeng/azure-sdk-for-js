...
2. **Project Folder Structure**: the typical structure is `sdk/{service-name}/{package-directory}`. For example, `sdk/keyvault/keyvault-keys` is a current package path in this repo. That folder is under `${SDK_REPO_ROOT}` and will be your **${PROJECT_ROOT}** folder.
...
1. **Configure `tspconfig.yaml` in the spec repository**
   ...
   options:
     "@azure-tools/typespec-ts":
       emitter-output-dir: "{output-dir}/{service-dir}/YOUR_PACKAGE_DIRECTORY"
       package-details:
...
2. **Generate code**
   ...
   **Re-generate an existing TypeScript DPG library**
   ...
   Run the `update` command from the SDK directory (e.g. `sdk/agrifood/agrifood-farming`):
