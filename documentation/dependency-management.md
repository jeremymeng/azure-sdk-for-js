...
Let's assume that a vulnerability has been found in [undici] version 5.28.3 and that you use `@azure/keyvault-keys` directly. Your security scan identified this vulnerability and asks to upgrade to 5.28.4 or higher.
...
First, let's see how `undici` is pulled into our dependency tree.
...
> If you are using Yarn you can use `yarn why undici` with similar results.

It looks like `undici`, a dependency of `@azure/core-rest-pipeline@1.16.0` is being pulled in _transitively_ via `@azure/keyvault-keys@4.10.0`.
...
As you can see undici has been updated to 5.28.4, without having to wait for a new version of `@azure/core-rest-pipeline`.
...
Not all updates are due to security vulnerabilities. Sometimes you just want to update a transitive dependency without deleting your lockfile. In that case, `npm update undici` can help. Let's see what that looks like:
...
> If you are using Yarn you can use `yarn up undici` with similar results.
