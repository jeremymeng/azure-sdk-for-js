...
1. Change to `sdk/<service-directory>/<package-directory>` if your current directory is not the package directory yet.
2. Run `pnpm lint`.
...
For linting issues that `lint:fix` could not resolve, you will need to examine the code and fix the issues accordingly.
...
For a generated package whose name starts with `@azure/arm-` or `@azure-rest/`, there might be `tsdoc/syntax` warnings because generated code files often contain some characters in reference docs that are not recommended in TSDoc. We should NEVER fix auto-generated files. You can add rules to suppress `tsdoc/syntax` in the package's `eslint.config.mjs`. Create that file by using one from other packages as a template, but do not copy rules that don't apply.
