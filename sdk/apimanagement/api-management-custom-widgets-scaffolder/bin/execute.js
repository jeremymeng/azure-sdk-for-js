...
const execute_helpers_1 = require("./execute-helpers");
const execute_configs_1 = require("./execute-configs");
...
async function main() {
    ...
    const getConfig = (0, execute_helpers_1.buildGetConfig)(gray, red);
    white("Specify the custom widget configuration.");
    const widgetConfig = await getConfig(execute_configs_1.promptWidgetConfig, execute_configs_1.validateWidgetConfig);
    white("Specify the Azure API Management service configuration.");
    const serviceInformation = await getConfig(execute_configs_1.promptServiceInformation, execute_configs_1.validateDeployConfig);
    white("Specify other options");
    const miscConfig = await getConfig(execute_configs_1.promptMiscConfig, execute_configs_1.validateMiscConfig);
    ...
    serviceInformation.managementApiEndpoint = (0, execute_configs_1.prefixUrlProtocol)(serviceInformation.managementApiEndpoint);
    miscConfig.openUrl = miscConfig.openUrl
        ? (0, execute_configs_1.prefixUrlProtocol)(miscConfig.openUrl)
        : miscConfig.openUrl;
    return (0, generateProject_1.generateProject)(widgetConfig, serviceInformation, miscConfig)
