...
describe("BlockBlobClient", () => {
  ...
  it("stageBlock", async () => {
    ...
    const listResponse = await blockBlobClient.getBlockList(
      Aborter.none,
      "uncommitted"
    );
  ...
  it("stageBlockFromURL copy source blob as single block", async () => {
    ...
    const listResponse = await newBlockBlobClient.getBlockList(
      Aborter.none,
      "uncommitted"
    );
  ...
  it("commitBlockList", async () => {
    ...
    await blockBlobClient.commitBlockList(Aborter.none, [
      base64encode("1"),
      base64encode("2")
    ]);
    const listResponse = await blockBlobClient.getBlockList(
