...
async function main() {
  ...
  // List containers
  const fileServiceClient = new BlobServiceClient(
  ...
  let marker;
  do {
    const listContainersResponse = await fileServiceClient.listContainersSegment(
      Aborter.none,
      marker
    );
  ...
  const containerClient = ContainerClient.fromBlobServiceClient(fileServiceClient, containerName);
