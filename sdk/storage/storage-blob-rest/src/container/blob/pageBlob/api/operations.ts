...
export function _copyIncrementalSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=incrementalcopy{?timeout}",
    {
...
export function _setSequenceNumberSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=properties&UpdateSequenceNumber{?timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      timeout: options?.timeout,
...
export function _resizeSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=properties&Resize{?timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      timeout: options?.timeout,
  ...
  return context.path(path).put({
    ...
    headers: {
      ...
      ...(options?.ifMatch !== undefined ? { "If-Match": options?.ifMatch } : {}),
      ...(options?.ifTags !== undefined ? { "x-ms-if-tags": options?.ifTags } : {}),
      "x-ms-blob-content-length": blobContentLength,
      ...options.requestOptions?.headers,
    },
  });
}
...
export async function resize(
  ...
  const result = await _resizeSend(context, blobContentLength, options);
...
export function _getPageRangesDiffSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=pagelist&diff{?snapshot,timeout,prevsnapshot,marker,maxresults}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
...
export async function _getPageRangesDiffDeserialize(
  ...
  return pageListDeserializer(result.body);
}
...
export function _getPageRangesSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=pagelist{?snapshot,timeout,marker,maxresults}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      snapshot: options?.snapshot,
...
export function _uploadPagesFromUrlSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=page&update&fromUrl{?timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      timeout: options?.timeout,
...
export function _clearPagesSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=page&clear{?timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      timeout: options?.timeout,
...
export function _uploadPagesSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=page&update{?timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      timeout: options?.timeout,
...
export async function create(
  ...
  const result = await _createSend(context, blobContentLength, options);
