...
export function _querySend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=query{?snapshot,timeout}",
    {
...
export function _getBlockListSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=blocklist{?snapshot,blocklisttype,timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      snapshot: options?.snapshot,
...
export async function _getBlockListDeserialize(result: PathUncheckedResponse): Promise<BlockList> {
  ...
  return blockListDeserializer(result.body);
}
...
export function _commitBlockListSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=blocklist{?timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      timeout: options?.timeout,
...
export function _stageBlockFromUrlSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=block&fromURL{?blockid,timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
...
export function _stageBlockSend(
  ...
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?comp=block{?blockid,timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      blockid: uint8ArrayToString(blockId, "base64"),
...
export function _putBlobFromUrlSend(
  context: Client,
  contentLength: number,
  copySource: string,
  options: PutBlobFromUrlOptionalParams = { requestOptions: {} },
): StreamableMethod {
  const path = expandUrlTemplate(
    "/{containerName}/{blobName}?BlockBlob&fromUrl{?timeout}",
    {
      containerName: context.containerName,
      blobName: context.blobName,
      timeout: options?.timeout,
...
export async function putBlobFromUrl(
  context: Client,
  contentLength: number,
  copySource: string,
  options: PutBlobFromUrlOptionalParams = { requestOptions: {} },
): Promise<void> {
  const result = await _putBlobFromUrlSend(context, contentLength, copySource, options);
  return _putBlobFromUrlDeserialize(result);
}
